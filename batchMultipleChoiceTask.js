  ...
  return new Promise((resolve, reject) => {
    ...
    docClient.scan(taskParams, (error, data) => {
      if (!error) {
        ...
        error.note =
          "The scan operation for the 'unfinished_task' table failed.";
        reject(error);
      }
    });
  });
  ...
  data.Items.forEach((item) => {
    ...
    Object.keys(item.class).forEach((className, occurrences) => {
      ...
      if (total - index + Object.entries(classes).length <= maxClasses) {
        ...
        Object.entries(classes).length < maxClasses
      ) {
        classes[className] = false;
      }
      index++;
...
function get(labelerId, response) {
  return new Promise((resolve, reject) => {
    getLabelerTaskList(labelerId, type)
      .then((data) => {
        ...
        let params = {
          ...
          ProjectionExpression:
            "instructions, #taskId, multiclass, #data, #class, #type",
          ExpressionAttributeNames: {
            "#taskId": "taskId",
            "#type": "type",
            "#data": "data",
        ...
        if (data.Count == 0) {
          ...
          params.FilterExpression =
            "NOT ( #taskId IN (" +
            Object.keys(taskObject).toString() +
            ") ) AND #type = :type";
        }
        ...
        getTasks(params, labelerId, taskCount, [])
          .then((tasks) => {
            response.headers = {
        ...
        if (!error) {
          ...
          if (data.Item.progress.current == data.Item.progress.total) {
            ...
            docClient.put(data, (error, _) => {
              if (error) {
                error.note =
                  "The put operation for the 'finished_task' table failed.";
                reject(error);
              }
            });
            ...
            docClient.delete(finishedTaskParams, (error, _) => {
              if (error) {
                error.note =
                  "The delete operation for the 'unfinished_task' table failed.";
                reject(error);
              }
            });
            ...
            docClient.update(datasetFinishedTaskParams, (error, _) => {
              if (error) {
                error.note =
                  "The update operation for the 'dataset_" +
                  data.Item.datasetId +
                  "' table failed.";
                reject(error);
              }
            ...
            let datasetProgressParams = {
              ...
              UpdateExpression:
                "SET #progress.#current = #progress.#current + :one",
              ExpressionAttributeNames: {
                "#progress": "progress",
                "#current": "current",
              },
            ...
            docClient.update(datasetProgressParams, (error, data) => {
              if (
                !error &&
                data.Item.progress.current == data.Item.progress.total
              ) {
                ...
                docClient.update(datasetFinishedParams, (error, _) => {
                  if (error) {
                    error.note =
                      "The second update operation for the 'dataset' table failed.";
                    reject(error);
                  } else {
            ...
            docClient.update(taskUpdateParams, (error, data) => {
              if (error) {
                error.note =
                  "The put operation for the 'unfinished_task' table failed.";
                reject(error);
              } else {
                // Resolve to indicate finished function
                resolve();
          ...
          error.note =
            "The get operation for the 'unfinished_task' table failed.";
          reject(error);
        }
      });
    });
...
exports.handler = async (request) => {
  ...
  return new Promise((resolve, reject) => {
    if (request.httpMethod == "GET") {
      get(request.pathParameters.labelerId, response)
        .then((taskResponse) => {
